...
class ResNetModelTest(tf.test.TestCase):
  ...
  def testGetScaledBaseLearningRateOneGpu(self):
    lr = self._test_scaled_base_learning_rate(1, 'parameter_server', 128)
  ...
  def testGetScaledBaseLearningRateTwoGpuUneven(self):
    lr = self._test_scaled_base_learning_rate(2, 'replicated', 13)
    self.assertEquals(lr, 0.0032500000000000003)

  def _test_scaled_base_learning_rate(self, num_gpus, variable_update,
